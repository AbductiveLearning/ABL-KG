...
def create_mined_rules(args, triples_fn, rule_fn):
    ...
    Test.main([triples_fn, "--minpca", "0.5", "--maxad", "3", "--minhc", "0.001", "--mins", "15", "--nc", "1"])
    jpype.shutdownJVM()
    

def simplify_mined_rules(rule_fn, simplified_rule_fn, pca=0.5, head_rels=set(), simple=False):
    rules = []
    rule_relations = []
    ...
    with open(simplified_rule_fn, "w") as f:
        ...
        for rule in rules:
            ...
            if pca_confidence >= pca:
                ...
                e3_var = rule_head[0]
                head_relations.add(rule_head[1])
                e4_var = rule_head[2]
                variable = set()
                for i in range(1, len(rule_body), 3):
                ...
                print(len({e1_var, e2_var, e3_var, e4_var}))
                if len({e1_var, e2_var, e3_var, e4_var}) == 3:  
                    # if rule_body_size != len(variable) - 1: 
                    # draw_one_rule(rule, "{}.png".format(index))
                    if len(head_rels) == 0:
                        f.write("{}\t=>\t{}\tpca:\t{}\n".format(rule_body, rule_head, pca_confidence))
                    elif rule_head[1] in head_rels:
                        if simple:
                            if len(rule_body) == 6 and rule_body[2] == rule_body[3]:
                                f.write("{} {} {} {}\n".format(rule_body[1], rule_body[4], rule_head[1], pca_confidence))
                        else:
                            f.write("{}\t=>\t{}\tpca:\t{}\n".format(rule_body, rule_head, pca_confidence))

    # with open("./dbpedia/head_relations.txt", "w") as f:
...
# create_mined_rules(args='', triples_fn='./dbpedia/selected_dbp_triples.txt', rule_fn='./dbpedia/rules.txt')
# simplify_mined_rules(rule_fn='./dbpedia/rules.txt', simplified_rule_fn='./dbpedia/selected_rules.txt')

# train_triples, _, _ = abl.read_relation_triples("./datasets/FB15K-237/train.txt")
# valid_triples, _, _ = abl.read_relation_triples("./datasets/FB15K-237/valid.txt")
# abl.triple_writer(train_triples | valid_triples, "./datasets/FB15K-237/triples.txt")
test_triples, _, _ = abl.read_relation_triples("./datasets/FB15K-237/test.txt")
relations = set(r for (h, r, t) in test_triples)

# create_mined_rules(args='', triples_fn="./datasets/FB15K-237/triples.txt", rule_fn="./datasets/FB15K-237/fb_rules.txt")
simplify_mined_rules(rule_fn="./datasets/FB15K-237/fb_rules.txt",
                     simplified_rule_fn="./datasets/FB15K-237/fb_simple_rules.txt",
                     pca=0.9, head_rels=relations, simple=True)
